...
<div class="wrap">
	<h1><?php esc_html_e( 'Content Find Replace', 'content-find-replace' ); ?></h1>

	<p><?php esc_html_e( 'Create unlimited rules, install the Wikimedia preset, and preview/apply safe migrations with rollback.', 'content-find-replace' ); ?></p>

	<h2><?php esc_html_e( 'Global Settings', 'content-find-replace' ); ?></h2>
	<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
		...
		<table class="form-table" role="presentation">
			<tr>
				<th scope="row"><?php esc_html_e( 'Plugin Enabled', 'content-find-replace' ); ?></th>
				<td><label><input type="checkbox" name="wcfr[enabled]" value="1" <?php checked( ! empty( $settings['enabled'] ) ); ?> /> <?php esc_html_e( 'Enable runtime replacement.', 'content-find-replace' ); ?></label></td>
			</tr>
			<tr>
				<th scope="row"><?php esc_html_e( 'Filter the_content', 'content-find-replace' ); ?></th>
				<td><label><input type="checkbox" name="wcfr[filter_content]" value="1" <?php checked( ! empty( $settings['filter_content'] ) ); ?> /> <?php esc_html_e( 'Apply rules to post content.', 'content-find-replace' ); ?></label></td>
			</tr>
			<tr>
				<th scope="row"><?php esc_html_e( 'Filter the_excerpt', 'content-find-replace' ); ?></th>
				<td><label><input type="checkbox" name="wcfr[filter_excerpt]" value="1" <?php checked( ! empty( $settings['filter_excerpt'] ) ); ?> /> <?php esc_html_e( 'Apply rules to excerpts.', 'content-find-replace' ); ?></label></td>
			</tr>
		</table>

		<h2><?php esc_html_e( 'Rules', 'content-find-replace' ); ?></h2>
		<p><?php esc_html_e( 'Rules run in the listed order. The Wikimedia preset appears as a strategy rule and can be toggled like others.', 'content-find-replace' ); ?></p>
			...
			<?php foreach ( $rules as $index => $rule ) : ?>
				...
				<div class="wcfr-rule-card">
					...
					<div class="wcfr-rule-header">
						<strong><?php echo esc_html( (string) ( $rule['name'] ?? __( 'Rule', 'content-find-replace' ) ) ); ?></strong>
						<button type="button" class="button button-small wcfr-remove-rule"><?php esc_html_e( 'Remove', 'content-find-replace' ); ?></button>
					</div>
					<p>
						<label><?php esc_html_e( 'Name', 'content-find-replace' ); ?><br />
					...
					<p>
						<label><?php esc_html_e( 'Strategy', 'content-find-replace' ); ?><br />
							<select class="wcfr-strategy-select" name="wcfr_rules[<?php echo esc_attr( (string) $row_index ); ?>][strategy]">
								<option value="find_replace" <?php selected( $strategy, 'find_replace' ); ?>><?php esc_html_e( 'Find/Replace', 'content-find-replace' ); ?></option>
								<option value="wikimedia_thumbnail_roundup" <?php selected( $strategy, 'wikimedia_thumbnail_roundup' ); ?>><?php esc_html_e( 'Wikimedia Thumbnail Roundup', 'content-find-replace' ); ?></option>
							</select>
						</label>
					</p>

					<div class="wcfr-preset-info<?php echo $is_preset ? '' : ' wcfr-hidden'; ?>">
						<strong><?php esc_html_e( 'How this rule works:', 'content-find-replace' ); ?></strong>
						<p><?php esc_html_e( 'Scans post content for Wikimedia thumbnail URLs and rewrites the pixel size to the next allowed size. No find/replace text is needed — the URL pattern and size ladder are built in.', 'content-find-replace' ); ?></p>
						<strong><?php esc_html_e( 'Matches URLs like:', 'content-find-replace' ); ?></strong>
						<code>//upload.wikimedia.org/wikipedia/commons/thumb/a/ab/File.png/240px-File.png</code>
						<strong><?php esc_html_e( 'Rewrites to next size from:', 'content-find-replace' ); ?></strong>
					...
					<div class="wcfr-rule-fields-fr<?php echo $is_preset ? ' wcfr-hidden' : ''; ?>">
						<p>
							<label><?php esc_html_e( 'Find', 'content-find-replace' ); ?><br />
						...
						<p>
							<label><?php esc_html_e( 'Replace', 'content-find-replace' ); ?><br />
					...
					<p class="wcfr-rule-flags<?php echo $is_preset ? ' wcfr-hidden' : ''; ?>">
						<label><input type="checkbox" name="wcfr_rules[<?php echo esc_attr( (string) $row_index ); ?>][use_regex]" value="1" <?php checked( ! empty( $rule['use_regex'] ) ); ?> /> <?php esc_html_e( 'Use regex', 'content-find-replace' ); ?></label>
						<label><input type="checkbox" name="wcfr_rules[<?php echo esc_attr( (string) $row_index ); ?>][ignore_case]" value="1" <?php checked( ! empty( $rule['ignore_case'] ) ); ?> /> <?php esc_html_e( 'Ignore case', 'content-find-replace' ); ?></label>
					</p>
					<p>
						<label><input type="checkbox" name="wcfr_rules[<?php echo esc_attr( (string) $row_index ); ?>][enabled]" value="1" <?php checked( ! empty( $rule['enabled'] ) ); ?> /> <?php esc_html_e( 'Enabled', 'content-find-replace' ); ?></label>
						<label><input type="checkbox" name="wcfr_rules[<?php echo esc_attr( (string) $row_index ); ?>][apply_in_admin]" value="1" <?php checked( ! empty( $rule['apply_in_admin'] ) ); ?> /> <?php esc_html_e( 'Apply in admin', 'content-find-replace' ); ?></label>
					</p>
					<p>
						<?php esc_html_e( 'Post Types (leave empty for all):', 'content-find-replace' ); ?><br />
		...
		<p>
			<button type="button" class="button" id="wcfr-add-rule"><?php esc_html_e( 'Add Rule', 'content-find-replace' ); ?></button>
			<?php if ( ! $has_wikimedia_preset ) : // phpcs:ignore -- $has_wikimedia_preset set in render_page() ?>
				<button type="button" class="button wcfr-add-wikimedia-preset" style="margin-left:8px;"><?php esc_html_e( 'Add Wikimedia Preset Rule', 'content-find-replace' ); ?></button>
			<?php else : ?>
				<span class="description" style="margin-left:12px;"><?php esc_html_e( 'Wikimedia Thumbnail Roundup rule is already in the list above.', 'content-find-replace' ); ?></span>
			<?php endif; ?>
		</p>

		<?php submit_button( __( 'Save Settings', 'content-find-replace' ) ); ?>
	</form>

	<hr />

	<h2><?php esc_html_e( 'Database Migration', 'content-find-replace' ); ?></h2>
	<p><?php esc_html_e( 'Preview changes before applying to the database. Rollback is available per run.', 'content-find-replace' ); ?></p>
	...
	<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" id="wcfr-migration-form">
		...
		<h3><?php esc_html_e( 'Rules to Apply', 'content-find-replace' ); ?></h3>
		<?php if ( ! empty( $rules ) ) : ?>
			<p class="description"><?php esc_html_e( 'Uncheck any rules you want to exclude from this migration run.', 'content-find-replace' ); ?></p>
			<?php foreach ( $rules as $rule ) : ?>
				...
				<label style="display:block;margin-bottom:6px;">
					...
					<?php echo esc_html( ( $rule['name'] ?? __( 'Unnamed rule', 'content-find-replace' ) ) . ' (' . ( $rule['strategy'] ?? 'find_replace' ) . ( empty( $rule['enabled'] ) ? ' — disabled' : '' ) . ')' ); ?>
				</label>
			<?php endforeach; ?>
		<?php else : ?>
			<p class="description"><?php esc_html_e( 'No rules configured yet. Add rules above first.', 'content-find-replace' ); ?></p>
		<?php endif; ?>

		<h3><?php esc_html_e( 'Scope', 'content-find-replace' ); ?></h3>
		<p>
			<label><?php esc_html_e( 'Limit posts scanned', 'content-find-replace' ); ?>
				<input type="number" min="1" max="500" name="wcfr_scope[limit]" value="100" />
			</label>
		</p>
		<p>
			<?php esc_html_e( 'Post types:', 'content-find-replace' ); ?><br />
			<?php foreach ( $post_types as $post_type_name => $post_type_obj ) : ?>
		...
		<p>
			<?php esc_html_e( 'Post statuses:', 'content-find-replace' ); ?><br />
		...
		<?php submit_button( __( 'Generate Preview', 'content-find-replace' ), 'secondary', 'submit', false ); ?>
	</form>
	...
	<?php if ( is_array( $preview ) ) : ?>
		<h3><?php esc_html_e( 'Latest Preview Results', 'content-find-replace' ); ?></h3>
		<p class="description">
			<?php
			printf(
				/* translators: %s: ISO8601 date string. */
				esc_html__( 'Generated at %s', 'content-find-replace' ),
		...
		<p>
			...
			printf( esc_html__( '%d post(s) will be affected.', 'content-find-replace' ), count( $preview['items'] ) );
			?>
		</p>
				...
				<div class="wcfr-preview-item">
					<div class="wcfr-preview-item-header">
						...
						<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" class="wcfr-apply-single-form">
							...
							<?php submit_button( __( 'Apply to this post', 'content-find-replace' ), 'small', 'submit', false, array( 'onclick' => "return confirm('Apply changes to this post now?');" ) ); ?>
						</form>
					</div>
								...
								if ( $remaining > 0 ) :
									...
									<p class="wcfr-more-occurrences">
										...
										printf(
											...
											esc_html__( '&hellip; and %d more occurrence(s) in this post.', 'content-find-replace' ),
											(int) $remaining
										);
										?>
									</p>
			...
			<?php endforeach; ?>
		<?php else : ?>
			<p><?php esc_html_e( 'No rewrite candidates found.', 'content-find-replace' ); ?></p>
		<?php endif; ?>
	<?php endif; ?>

	<h3><?php esc_html_e( 'Available Rollbacks', 'content-find-replace' ); ?></h3>
	<?php if ( ! empty( $runs ) ) : ?>
		<table class="widefat striped">
			<thead><tr><th><?php esc_html_e( 'Run ID', 'content-find-replace' ); ?></th><th><?php esc_html_e( 'Created', 'content-find-replace' ); ?></th><th><?php esc_html_e( 'Updated IDs', 'content-find-replace' ); ?></th><th><?php esc_html_e( 'Action', 'content-find-replace' ); ?></th></tr></thead>
			<tbody>
					...
					<tr>
						...
						<td>
							<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
								...
								<?php submit_button( __( 'Rollback', 'content-find-replace' ), 'delete', 'submit', false, array( 'onclick' => "return confirm('Rollback this migration run?');" ) ); ?>
							</form>
						</td>
					</tr>
				<?php endforeach; ?>
	...
	<?php else : ?>
		<p><?php esc_html_e( 'No rollback snapshots yet.', 'content-find-replace' ); ?></p>
	<?php endif; ?>
</div>
